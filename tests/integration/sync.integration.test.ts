...
describe("sync", () => {
  ...
  beforeAll(() => {
    sql = postgres(integrationContext().databaseUrl);
  });
  afterAll(async () => {
    await sql.end();
  });
  ...
  it("fails closed when the authenticated user has no entitlement", async () => {
    const response = await sync("/api/sync", crypto.randomUUID(), {
      since: 0,
      changes: [],
    });
  ...
  it("applies and pulls a record, then rejects an equal-timestamp update through the legacy alias", async () => {
    ...
    const appliedResponse = await sync("/api/sync", userId, {
      since: 0,
  ...
  it("keeps records isolated by user and synchronizes tombstones", async () => {
    ...
    const ownerResponse = await sync("/api/sync", ownerId, {
      since: 0,
      changes: [tombstone],
    });
    ...
    const otherResponse = await sync("/api/sync", otherUserId, {
      since: 0,
      changes: [],
    });

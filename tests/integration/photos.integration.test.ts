...
const uploadPhoto = (userId: string, beanId: string, updatedAt: number, photo: Uint8Array) =>
...
describe('photos', () => {
	it('uploads, lists, downloads, and deletes an entitled bean photo', async () => {
		...
		const manifest = await fetch(apiUrl('/api/photos'), { headers: authenticatedHeaders(userId) });
		expect(await manifest.json()).toEqual({
			photos: [{ beanId, updatedAt, deleted: false, mimeType: 'image/jpeg' }]
		});

		const download = await fetch(apiUrl(`/api/photos/${beanId}`), { headers: authenticatedHeaders(userId) });
		...
		const remove = await fetch(apiUrl(`/api/photos/${beanId}`), {
			method: 'DELETE',
			headers: authenticatedHeaders(userId, { 'x-photo-updated-at': String(updatedAt + 1) })
		});
		expect(remove.status).toBe(200);
		expect(await remove.json()).toEqual({
			applied: true,
			photo: { beanId, updatedAt: updatedAt + 1, deleted: true, mimeType: null }
		});

		const afterDelete = await fetch(apiUrl(`/api/photos/${beanId}`), { headers: authenticatedHeaders(userId) });
	...
	it('requires authentication and entitlement and validates bean identifiers', async () => {
		...
		const userId = crypto.randomUUID();
		const unentitled = await fetch(apiUrl('/api/photos'), { headers: authenticatedHeaders(userId) });
		expect(unentitled.status).toBe(403);
		expect(await unentitled.json()).toEqual({ error: 'subscription_required' });

		await grantPhotoAccess(userId);
		const invalidBean = await fetch(apiUrl('/api/photos/invalid!'), { headers: authenticatedHeaders(userId) });
	...
	it('keeps the newer server photo when an older upload arrives', async () => {
		...
		expect(await staleUpload.json()).toEqual({
			applied: false,
			photo: { beanId, updatedAt: 10_000, deleted: false, mimeType: 'image/jpeg' }
		});

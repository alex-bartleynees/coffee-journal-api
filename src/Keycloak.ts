	...
	Effect.gen(function* () {
		...
		const token = Effect.tryPromise({
			try: async () => {
				...
				if (!response.ok) throw new Error(`service token request returned ${response.status}`);

				const body = (await response.json()) as TokenResponse;
				if (typeof body.access_token !== 'string') throw new Error('service token response was invalid');
		...
		const createUser: KeycloakService['createUser'] = (user) =>
			Effect.gen(function* () {
				...
				const firstName = firstSpace === -1 ? normalizedName : normalizedName.slice(0, firstSpace);
				const lastName = firstSpace === -1 ? '' : normalizedName.slice(firstSpace + 1);
				...
				const search = yield* Effect.tryPromise({
					try: () =>
						fetchWithTimeout(`${endpoint}?email=${encodeURIComponent(user.email)}&exact=true`, {
							headers: { authorization: `Bearer ${bearer}` }
						}),
					catch: (cause) => new KeycloakUnavailableError({ reason: String(cause) })
				});
				if (!search.ok) {
					return yield* new KeycloakUnavailableError({ reason: `user lookup returned ${search.status}` });
				}

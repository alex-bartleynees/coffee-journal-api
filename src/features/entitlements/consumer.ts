	...
	Effect.gen(function* () {
		...
		yield* Effect.tryPromise({
			try: async () => {
				...
				await channel.assertExchange(DLX, 'direct', { durable: true, autoDelete: false });
		...
		yield* Effect.async<never, Error>((resume) => {
			connection.on('close', () => resume(Effect.fail(new Error('amqp connection closed'))));
			connection.on('error', () => {
	...
	Effect.gen(function* () {
		...
		yield* Effect.forkDaemon(
			consumeSession(url).pipe(
				Effect.tapError((e) => Effect.logWarning(`[entitlements] session failed: ${e.message}`)),
				Effect.retry(

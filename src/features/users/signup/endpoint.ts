...
export const signupEndpoint = Effect.gen(function* () {
	...
	return yield* HttpServerResponse.schemaJson(CreateUserResponse)(response, { status: 201 });
}).pipe(
	HttpServerRequest.withMaxBodySize(Option.some(FileSystem.KiB(4))),
	Effect.catchTags({
		SignupRateLimited: () => HttpServerResponse.json({ error: 'too_many_requests' }, { status: 429 }),
		InvalidSignup: () => HttpServerResponse.json({ error: 'invalid_request' }, { status: 400 }),

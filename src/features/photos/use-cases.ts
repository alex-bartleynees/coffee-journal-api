...
const bestEffortDelete = (storage: PhotoStorage['Type'], key: string) =>
	storage.delete(key).pipe(
		Effect.catchAll((cause) =>
			Effect.logWarning('Failed to remove superseded photo object', { key, cause }))
	);
...
export const getPhoto = (userId: string, beanId: string) =>
	Effect.gen(function* () {
		...
		if (!photo || photo.deleted || !photo.objectKey || !photo.mimeType) {
			return yield* new PhotoRequestError({ status: 404, code: 'photo_not_found' });
		}
		return { bytes: yield* storage.get(photo.objectKey), mimeType: photo.mimeType };
	});

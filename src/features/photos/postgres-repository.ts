...
import { PhotoRepository, type PhotoRepositoryService, type StoredPhoto } from './repository.js';
		...
		const apply: PhotoRepositoryService['apply'] = (userId, photo, objectKey) =>
			Effect.tryPromise({
				try: () => sql.begin(async (tx) => {
					...
					const currentRows = applied ? rows : await tx<PhotoRow[]>`
						SELECT bean_id, updated_at, deleted, mime_type, object_key FROM bean_photos
						WHERE user_id = ${userId} AND bean_id = ${photo.beanId}`;
					return {
						applied,
						current: toStoredPhoto(currentRows[0]!),
						previousObjectKey: applied ? (before[0]?.object_key ?? null) : null
					};
				}) as Promise<{
					applied: boolean;
